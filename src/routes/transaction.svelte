<script lang="ts">
  ...
  import Search from '../components/Search.svelte';
	...
	onMount(async () => {
		...
		try {
			receipt = await provider.getTransactionReceipt(id!);
			txn = await provider.getTransaction(id!);
								...
								<Table responsive borderless>
									<tbody>
										<tr>
											...
											<td
												><a href="/address?id={receipt.from}">{receipt.from}</a>
												{#if senderENS}
													({senderENS})
												{/if}
											</td>
										...
										<tr>
											...
											{#if receipt.to}
												<td
													><a href="/address?id={receipt.to}">{receipt.to}</a>
													{#if receiverENS}
														({receiverENS})
													{/if}
											...
											{:else if receipt.contractAddress}
												<td
													><a href="/address?id={receipt.contractAddress}"
									...
									<Table responsive borderless>
										<tbody>
											...
											<tr>
												...
												<td><a href="/address?id={log.address}">{log.address}</a></td>
											</tr>
											<tr>
												<th class="titleWidth" scope="row">Data</th>
